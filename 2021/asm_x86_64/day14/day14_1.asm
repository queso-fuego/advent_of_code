...
main:
    ...
    .loop:
        ...
        imul rbx, rbx, 26*4
        add r12, rbx
        inc dword [letter_pair_counts+r12] 
    ...
    add_letter_counts_loop:
        ...
        .inner_loop:
            ...
            movsx rax, dword [r10]
            add [r14], eax              ; letter_counts[i] += letter pairs[i][j]
    ...
    print_letter_counts:
        ...
        imul r13, r12, 4
        movsx rsi, dword [letter_counts+r13]
        cmp rsi, 0
        je .print                   ; Skip checking 0 values

        cmp esi, [most_common]
        jbe .check_least            ; Unsigned comparison
        mov [most_common], esi      ; Set new max

        .check_least:
        cmp esi, [least_common]
        jae .print                  ; Unsigned comparison
        mov [least_common], esi     ; Set new min
    ...
    movsx rsi, dword [most_common]
    sub esi, dword [least_common]
    call print_int
...
apply_rules:
    ...
    .loop:
        ...
        imul r15, r15, 26*4
        add r14, r15
        movsx rax, dword [letter_pair_counts+r14]   ; Number of this pair in array
        ...
        imul r15, r15, 26*4
        add r14, r15
        add [letter_pairs_copy+r14], eax    ; Add number of original pairs to 1st new pair
        ...
        imul r15, r15, 26*4
        add r14, r15
        add [letter_pairs_copy+r14], eax    ; Add number of original pairs to 2nd new pair
...
print_table:
    ...
    .loop2_outer:
        ...
        .loop2_inner:
            push rsp
            movsx rsi, dword [letter_pair_counts+r14]
            call print_int
            pop rsp
...
section .data
letter_pair_counts: times 26*26 dd 0    ; Array of all letter combinations
letter_pairs_copy: times 26*26 dd 0    ; Array of all letter combinations
template: times 26 db 0         ; Initial string
pair_rules: times 3*100 db 0    ; 2 Characters + insert character
num_rules: db 0
letter_counts: times 26 dd 0
most_common: dd 0
least_common: dd 0xFFFFFFFF
